...
(define (rotate left right Acc)
  (cond
    ((stream-empty? left) (stream-append right Acc))
    (else (stream-cons (stream-first left) (rotate (stream-rest left) (stream-rest right) (stream-cons (stream-first right) Acc))))))
...
(define (enqueue x q)
  (cond
  ((>= (queue-size-l q) (+ (queue-size-r q) 1))(struct-copy queue q [right (append (list x) (queue-right q))] [size-r (+ (queue-size-r q) 1)]))
  (else (struct-copy queue q [left (rotate (queue-left q) (append (list x) (queue-right q)) empty-stream)][right null] [size-l (+ (+ (queue-size-l q) 1)(queue-size-r q))][size-r 0]))))
...
(define (dequeue q)
  (cond
    ((>= (- (queue-size-l q) 1) (queue-size-r q)) (struct-copy queue q [left (stream-rest (queue-left q))][size-l (- (queue-size-l q) 1)]))
    (else (struct-copy queue q [left (rotate (stream-rest (queue-left q)) (queue-right q) empty-stream)][right null][size-l (- (+ (queue-size-l q) (queue-size-r q)) 1)][size-r 0]))))
